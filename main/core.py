...
import sys, pygame, math, random, time, copy, os
import numpy as np
...
file = os.path.join('results', strftime("%Y%m%d-%H%M%S", gmtime()))
handle = open(file, 'w')
# handle.write("NUMANTS: " + str(NUMANTS) + "\nMAPTYPE: " + "\nALGORITHM:" + str(ALGO) + "\n\n")
...
class Mover(pygame.sprite.Sprite, Thing):
    ...
    def turnToFace(self, pos):
        ...
        angle = math.degrees(np.arctan2(direction[0], direction[1])) - 90
...
class SimpleResource(Resource):
    ...
    def collision(self, thing):
        ...
        if isinstance(thing, Agent) and thing.foodAmount == 0:
            thing.foodAmount += 1
...
class Bullet(Mover):
    ...
    def hit(self, thing):
        if thing != self.owner and isinstance(thing, Agent) and (
                thing.getTeam() == None or thing.getTeam() != self.owner.getTeam()):
            thing.damage(self.damage)
            return True
        elif isinstance(thing, Obstacle) or isinstance(thing, Gate) or self.position[0] < 0 or self.position[0] > \
                self.world.dimensions[0] or self.position[1] < 0 or self.position[1] > self.world.dimensions[1]:
            return True
        else:
            return False
...
class Agent(Mover):
    ...
    ### Constructor
    def __init__(self, image, position, orientation, speed, world, hitpoints=HITPOINTS, firerate=FIRERATE,
                 bulletclass=Bullet):
        Mover.__init__(self, image, position, orientation, speed, world)
    ...
    def update(self, delta):
        ...
        if self.moveTarget is not None:
            ...
            if self.navigator != None:
                self.navigator.updateDestination()
        # except:
        # 	pass
...
class Obstacle(Thing, Blocker):
    ...
    def twoAdjacentPoints(self, p1, p2):
        if self.isInPoints(p1) and self.isInPoints(p2):
            return (abs(self.points.index(p1) - self.points.index(p2)) == 1) or (
            p1 == self.points[0] and p2 == self.points[len(self.points) - 1]) or (
                   p2 == self.points[0] and p1 == self.points[len(self.points) - 1])
        else:
            return False
        ...
        if sprite is not None:
            ...
            for x in range((self.rect.width * 2) // dec.rect.width):
                for y in range((self.rect.height * 2) // dec.rect.height):
                    pos = (((x / 2) * dec.rect.width) + corerandom.uniform(0, dec.rect.width / 5.0),
                           ((y / 2) * dec.rect.height) + corerandom.uniform(0, dec.rect.height / 5.0))
                    orient = corerandom.uniform(0, 360.0)
                    if pointInsidePolygonPoints((pos[0] + dec.rect.width / 2.0, pos[1] + dec.rect.height / 2.0),
                                                points):
...
class GameWorld():
    ...
    def __init__(self, seed, worlddimensions, screendimensions):
        ...
        self.clock = 0
        # camera
        self.camera = [self.dimensions[0] / 2, self.dimensions[1] / 2]
        # unobstructed places
        self.destinations = {}
        # pheromones
        self.markers = []
        self.leaving_markers = []
    ...
    def initializeNest(self):
        self.nest = pygame.sprite.Sprite()
        self.nest.image = pygame.image.load(TOWER).convert()
        self.nest.rect = self.nest.image.get_rect()
        self.nest.rect.center = [WORLD[0]/2, WORLD[1]/2]
    ...
    def initializeRandomTerrain(self, num, onum, radius, sigma, min):
        ...
        lines = [((0, 0), (self.dimensions[0], 0)), ((self.dimensions[0], 0), (self.dimensions[0], self.dimensions[1])),
                 ((self.dimensions[0], self.dimensions[1]), (0, self.dimensions[1])), ((0, self.dimensions[1]), (0, 0))]
        for _ in range(num):
            pos = [0, 0]
            for _ in range(100):
                pos = [corerandom.randint(0, self.dimensions[0] - radius),
    ...
    def initializeBaselineResources(self, num, max_cluster=6, resource=RESOURCE):
        rng = np.random.RandomState(8)
        for _ in range(num):
            pos = (0, 0)
            while True:
                pos = (rng.randint(0, self.dimensions[0]), rng.randint(0, self.dimensions[1]))
                if foodInObstacle(self,pos) == False:
                    break
            parts = rng.randint(0, max_cluster)
            for _ in range(parts):
                offset = rng.multivariate_normal([0, 0], cov=20 * np.eye(2))
                r = SimpleResource(resource, (pos[0]+offset[0],pos[1]+offset[1]), 0, self)
                self.addResource(r)

    def initializeRandomResources(self, num, max_cluster=6, resource=RESOURCE):
        for _ in range(num):
            pos = (0, 0)
            while True:
                ...
                if foodInObstacle(self,pos) == False:
                    break
            parts = random.randint(0, max_cluster)
            for _ in range(parts):
                offset = np.random.multivariate_normal([0, 0], cov=20 * np.eye(2))
                r = SimpleResource(resource, (pos[0]+offset[0],pos[1]+offset[1]), 0, self)
                self.addResource(r)

    def initializeRandomResourcesOrigin(self, num, resource=RESOURCE):
        for _ in range(num):
            pos = (0, 0)
            while True:
                pos = (corerandom.randint(0, self.dimensions[0]), corerandom.randint(0, self.dimensions[1]))
    ...
    def run(self):
        ...
        while True:
            ...
            self.decayMarkers()
            self.drawWorld()
            handle.write(str(len(self.resources)) + "\n")
            handle.flush()
            pygame.display.flip()
            # good for taking screen shots at particular ticks
            #if i == 3:
            #    import time
            #    time.sleep(100)
            i += 1

    def decayMarkers(self):
        new_markers = []
        for loc, timeToLive in self.markers:
            if timeToLive <= 0:
                continue
            new_markers.append((loc, timeToLive-1))
            coord = numpyArrayToCoordinates(loc, self.dimensions)
            drawCross(self.background, coord, (0, 0, 200), 5)
        self.markers = new_markers

        new_markers = []
        for loc, timeToLive in self.leaving_markers:
            if timeToLive <= 0:
                continue
            new_markers.append((loc, timeToLive-1))
            coord = numpyArrayToCoordinates(loc, self.dimensions)
            #drawCross(self.background, coord, (200, 0, 0), 5)
        self.leaving_markers = new_markers
    ...
    def doKeyDown(self, key):
        ...
        if key == 276: # left
            self.agent.moveToTarget((x - step, y))
        elif key == 275:
            self.agent.moveToTarget((x + step, y))
        elif key == 273:
            self.agent.moveToTarget((x, y - step))
        elif key == 274:
            self.agent.moveToTarget((x, y + step))
            ...
            if m1 in self.movers:
                ...
                for o in self.obstacles:
                    ...
                    if isinstance(m1, Agent) and m1.moveTarget != None:
                        p = rayTraceWorldNoEndPoints(m1.position, m1.moveTarget, o.getLines())
                        if p == None:
                            needCheckVertex = True
                    if needCheckVertex:
                        for v in o.getPoints():
                            # check v between m1.position and m1.moveTarget
                            if (v[0] - m1.position[0]) * (v[0] - m1.moveTarget[0]) < 0 and (v[1] - m1.position[1]) * (v[1] - m1.moveTarget[1]) < 0:
                                d = minimumDistance((m1.position, m1.moveTarget), v)
                                if d < distance(m1.rect.topleft, m1.rect.topright) / 2:
                                    needCheckVertex = False
    ...
    def collisionWithNonMover(self, position, rect):
        # TODO: implement world collisions in terms of this?
        # Collision against world boundaries
        if position[0] < 0 or position[0] > self.dimensions[0] or \
           position[1] < 0 or position[1] > self.dimensions[1]:
            return True
        # Collision against obstacles
        for o in self.obstacles:
            for l in o.getLines():
                for r in ((rect.topleft, rect.topright), (rect.topright, rect.bottomright),
...
class Gate(Thing, Blocker):
    def __init__(self, p1, p2, sprite, world):
        ...
        for t in range(length / size):
            pos = (p1[0] + ((float(t) / length) * size * (p2[0] - p1[0])),
                   p1[1] + ((float(t) / length) * size * (p2[1] - p1[1])))
...
class GatedWorld(GameWorld):
    ...
    def makePotentialGates(self):
        if self.obstacles != None:
            dangerpoints = [(0, 0), (self.dimensions[0], 0), (self.dimensions[0], self.dimensions[1]),
                            (0, self.dimensions[1])]
